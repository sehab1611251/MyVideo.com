...
<form method="post" action="video.php?id=<?php echo $id?>">
 ...
 <input type="submit" name="playlist" id="playlist" class="btn btn-primary btn-sm pull-right" value="Add to playlist"> 
</form>
...
<?php
$id=$_GET['id'];
$comment_sql="SELECT userinfo.username, comment.comment FROM userinfo
INNER JOIN comment ON userinfo.id = comment.user_id where comment.video_id = $id and comment.user_id = $user_id";
